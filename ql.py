...
def list_awards(game_id, uid, cred):
    resp = requests.get(sign_url, headers=copy_header(cred), params={'gameId': game_id, 'uid': uid}).json()
    print(resp)
...
def do_sign(cred_resp):
    ...
    for i in characters:
        ...
        resp = requests.post(sign_url, headers=get_sign_header(sign_url, 'post', body, copy_header(cred_resp)),
                             json=body).json()
        if resp['code'] != 0:
